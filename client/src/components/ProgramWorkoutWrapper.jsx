...
const ProgramWorkoutWrapper = ({ weeks, isActiveProgram }) => {
  ...
  const handleCompletedToggle = async (weekId, dayId, workoutId) => {
    ...
    const dayIndex = newWeekData[weekIndex].days.findIndex(day => day._id === dayId);
    if (dayIndex === -1) return; // day not found
  
    const workoutIndex = newWeekData[weekIndex].days[dayIndex].workouts.findIndex(workout => workout._id === workoutId);
    if (workoutIndex === -1) return; // workout not found
  
    newWeekData[weekIndex].days[dayIndex].workouts[workoutIndex] = updatedDay.workouts.find(workout => workout._id === workoutId);
